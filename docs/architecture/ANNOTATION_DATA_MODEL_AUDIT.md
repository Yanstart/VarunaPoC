...
| ~~**G2**~~ ✅ **RESOLVED** par #370 | ~~Pas de registre `ml_models`~~ — table `ml_models` créée (migration 010), 7 endpoints CRUD, seeder Phikon-v2, audit hooks. Voir `docs/Admin/services/ml-models-registry.md`. | Ambiguïtés (`"resnet50"` vs `"ResNet50"`), pas de FK, pas de hash de checkpoint |
