    ...
    async def get_stats(self, db_session, model_name: str = None) -> list:
        ...
        query = (
            select(
                ...
                func.sum(sql_case((Correction.correction_type == "confirmed", 1), else_=0)).label("confirmed"),
                func.sum(sql_case((Correction.correction_type == "rejected", 1), else_=0)).label("rejected"),
                func.sum(sql_case((Correction.correction_type == "refined", 1), else_=0)).label("refined"),
                func.sum(sql_case((Correction.correction_type == "relabeled", 1), else_=0)).label("relabeled"),
            )
